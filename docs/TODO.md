...
## Pre-deploy (must do before sharing the URL)

- [ ] **Move Claude calls behind Firebase Functions proxy.** Browser-direct with `VITE_ANTHROPIC_API_KEY` is dev-only. Do not deploy a build that calls Anthropic directly.
...
- [ ] **Set Anthropic budget alerts** in the Anthropic console (e.g., $5/$10/$25 thresholds).
...
- [ ] **Coach trigger timing.** Defaults are 3s idle / 15s rate-limit floor. Likely needs adjustment based on how chatty vs sleepy it feels. Consider:
  - Bumping idle to 5s if the coach interrupts mid-thought too often.
  - Lowering 15s floor to 10s for novices who benefit from more frequent feedback.
...
- [ ] **Stroke brush width.** Spec uses 8 logical units. May need to scale with `viewBox` size or feel of the canvas.
- [ ] **Backing track auto-pause** when the tab loses focus, to be polite about background CPU + audio.

## Browser quirks

- [ ] **Pressure on Mac trackpad.** Test PointerEvent.pressure on Safari, Chrome, Firefox. Add `webkitForce` fallback for Safari. Document what works where.
...
Tracking here just so they don't leak into v1: stroke-level eraser, tracing layer, replay, stroke move, velocity-based pressure, line width / color settings, leveled principle library, skill assessment, before/after comparison, challenge mode, warm-ups, adaptive step granularity.
